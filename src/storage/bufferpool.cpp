...
u_int16_t BufferPool::createNewPage(bool is_leaf, File& file,
                                    uint16_t rightMostChildPageId) {
  u_int16_t pageID = file.allocateNextPageId();
  int frame_id = obtainFreeFrame();
  char* frame_p =
      static_cast<char*>(buffer) + frame_id * BufferPool::FRAME_SIZE_BYTE;

  auto page =
      std::make_unique<Page>(frame_p, is_leaf, rightMostChildPageId, pageID);
  Page* page_ptr = page.get();
  frameDirectory_.registerPage(frame_id, pageID, file.getFilePath(),
                               std::move(page));
  LOG_INFO("Created new page ID {} as {} page in frame ID {}", pageID,
...
void BufferPool::unpin(Page* page, File& file) {
  ...
  auto it =
      frameDirectory_.findFrameByPage(page->getPageID(), file.getFilePath());
  if (!it.has_value()) {
    throw std::logic_error(
...
void BufferPool::zeroOutFrame(int frameID) {
  ...
  char* frame_p =
      static_cast<char*>(buffer) + frameID * BufferPool::FRAME_SIZE_BYTE;
