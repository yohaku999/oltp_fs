...
class IntermediateCell : public Cell {
 private:
  // The value range of cell key, pageID is 0~4095 for now, so we can use
  // uint16_t to store them.
  static constexpr size_t KEY_SIZE_BYTE = sizeof(int);
  static constexpr size_t PAYLOAD_SIZE = Cell::FLAG_FIELD_SIZE +
 ...
 public:
  ...
  IntermediateCell(uint16_t page_id, int key)
      : key_size_(KEY_SIZE_BYTE), page_id_(page_id), key_(key) {}
};