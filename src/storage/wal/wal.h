...
class WAL {
 public:
  ...
  //   and is done outside page latches, so it is unlikely to dominate
  //   throughput under our expected workloads (single-socket, <= 32 threads,
  //   moderate record sizes).
  ...
  //   currently treat cache-line contention on that counter as a primary
  //   concern.
  //
  // If profiling later shows that either the global LSN counter or
  // serialization becomes a measurable bottleneck, we can move both
  // responsibilities to a dedicated WAL thread without changing the on-disk
  // format.
 ...
 private:
  ...
  // "mutable" allows getFlushedLSN() to acquire the mutex even though it's a
  // const method. it is allowed because the mutex is only used to protect the
  // internal state of WAL and does not affect the observable behavior of WAL
  // from the caller's perspective. protects all shared state in WAL: buffer_,
  // last_lsn_written_, and flushed_lsn_.
  ...
  // LSN of the last record that is known to be durable　(its bytes have been
