 ...
 * - RecordCellView is a non-owning view over heap-record bytes already stored in a page
 * - RecordBuilder owns temporary bytes while constructing a record for insertion
...
public:
    ...
    {
        uint32_t null_bitmap = readValue<uint32_t>(cell_start_ + Cell::FLAG_FIELD_SIZE + sizeof(uint16_t));
    ...
    {
        return cell_start_ + readValue<uint16_t>(cell_start_ + Cell::FLAG_FIELD_SIZE);
    }

    std::pair<const char *, uint16_t> getXthVariableColumnbegin(int x, int variable_column_count) const
    {
        const char *variable_payload_begin = getVariableLengthPayloadBegin();
        const char *value_bytes_begin = variable_payload_begin + sizeof(uint16_t) * variable_column_count;

        uint16_t column_begin_offset = 0;
        if (x > 0)
        {
            const char *previous_end_offset_entry = variable_payload_begin + sizeof(uint16_t) * (x - 1);
        ...
        return {
            value_bytes_begin + column_begin_offset,
