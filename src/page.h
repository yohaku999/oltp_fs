...
private:
    ...
    // TODO: add is_leaf_node_ to the header of page, so that we can determine whether the page is a leaf node or an intermediate node by reading the header of page, instead of relying on the external information. For now, we just use an instance variable to indicate whether the page is a leaf node or an intermediate node, but we can move it to the header of page in the future if needed.
...
public:
    ...
    static Page* initializePage(char *start_p, bool is_leaf_node);
