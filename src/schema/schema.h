...
public:
    Schema(std::string name, std::vector<Column> columns) : name_(std::move(name)), columns_(std::move(columns)) {}
    ...
    // Sum of sizes of fixed-length columns from index 0 up to (but excluding) fixed_index.
