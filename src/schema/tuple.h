...
class Tuple {
 public:
  static std::string getValuesAsString(const char* cell_start,
    ...
    // get value
    // for fixed length, we can directly read the value based on the offset of
    // the value in the record cell.
    if (target_column->isFixedLength()) {
      ...
      std::tie(target_column_begin, target_column_size) =
          view.getXthVariableColumnbegin(target_column_index,
                                         schema.getVariableColumnCount());
      // convert to real value.
