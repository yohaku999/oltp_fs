...
class ExchangeCoordinator {
 public:
  ...
  ExchangeCoordinator(
      ...
      std::optional<std::function<size_t(const T&)>> partition_hash_fn =
          std::nullopt) {
    ...
    for (size_t i = 0; i < consumer_threads_count; i++) {
      ...
      consumers_.push_back(
          std::make_unique<ExchangeConsumerOperator<T>>(*queue));
    ...
    if (dispatch_rule == DispatchRule::RoundRobin) {
      ...
      sink = std::make_shared<ExchangeSink<T>>(std::move(dispatcher), queues_,
                                               producer_threads_count);
    } else if (dispatch_rule == DispatchRule::HashPartition) {
      if (!partition_hash_fn.has_value()) {
        throw std::invalid_argument(
            "partition_hash_fn is required for HashPartition dispatch");
      }
      auto dispatcher = std::make_unique<HashPartitionDispatcher<T>>(
  ...
  size_t consumerCount() const { return consumers_.size(); }
