...
class ExchangeDispatcher {
 public:
  ...
  virtual void dispatch(const std::vector<T>& batch,
...
class RoundRobinDispatcher : public ExchangeDispatcher<T> {
 public:
  void dispatch(const std::vector<T>& batch,
...
class HashPartitionDispatcher : public ExchangeDispatcher<T> {
 public:
  explicit HashPartitionDispatcher(std::function<size_t(const T&)> partition_hash_fn)
  ...
  void dispatch(const std::vector<T>& batch,
    ...
    for (size_t partition_id = 0; partition_id < partitioned_batches.size(); ++partition_id) {
