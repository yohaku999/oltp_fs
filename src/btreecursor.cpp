...
{
    // traverse the btree from the root page until we find the leaf page that may contain the key.
...
{
    LOG_INFO("Inserting index entry for key {} pointing to heap page ID {}, slot ID {} into index file {}.",
    ...
    auto leaf_slot_id = leafPage->insertCell(LeafCell(key, heap_page_id, slot_id));
