...
TEST_F(ExecutorTest, InsertAndGetMultipleRecords)
{
    ...
    for (const auto &record : records)
    {
        ...
        executor::insert(*pool_, *index_file_, *heap_file_, record.first, value_ptr, record.second.size());
    }

    // read
    for (const auto &record : records)
    {
        char *stored = executor::read(*pool_, *index_file_, *heap_file_, record.first);
...
TEST_F(ExecutorTest, InsertDeleteThenFailToRead)
{
    ...
    executor::insert(*pool_, *index_file_, *heap_file_, key, value_ptr, payload.size());

    executor::remove(*pool_, *index_file_, *heap_file_, key);

    // reading should now fail because the slot has been invalidated
    EXPECT_THROW({ executor::read(*pool_, *index_file_, *heap_file_, key); }, std::runtime_error);
}
...
TEST_F(ExecutorTest, UpdateReplacesExistingValue)
{
    ...
    executor::insert(*pool_, *index_file_, *heap_file_, key, initial_ptr, initial.size());

    char* update_ptr = updated.data();
    executor::update(*pool_, *index_file_, *heap_file_, key, update_ptr, updated.size());
...
TEST_F(ExecutorTest, UpdateNonExistingKeyThrows)
{
    ...
    EXPECT_THROW(
        {
            executor::update(*pool_, *index_file_, *heap_file_, key, value_ptr, payload.size());
        },
        std::runtime_error);
}
...
TEST_F(ExecutorTest, InsertPageOverflow)
{
    try
    {
        ...
        for (size_t attempt = 0; attempt < max_attempts; ++attempt)
        {
            ...
            executor::insert(*pool_, *index_file_, *heap_file_, key, payload.data(), payload.size());
    ...
    {
        std::cout << "Executor InsertPageOverflow test threw exception: " << ex.what() << std::endl;
