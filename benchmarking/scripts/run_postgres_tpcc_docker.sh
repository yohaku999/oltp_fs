...
HOST_DBFS_BUFFER_POOL_EVENT_LOG_FILE="${HOST_RESULTS_DIR}/dbfs.buffer_pool_events.log"
...
DBFS_BUFFER_POOL_EVENT_LOG_FILE_CONTAINER="${ARTIFACTS_DIR_CONTAINER}/dbfs.buffer_pool_events.log"
...
if [[ "${ENGINE}" == "dbfs" ]]; then
  ...
  export DBFS_BUFFER_POOL_EVENT_LOG_FILE="${DBFS_BUFFER_POOL_EVENT_LOG_FILE:-${DBFS_BUFFER_POOL_EVENT_LOG_FILE_CONTAINER}}"
  rm -f "${HOST_DBFS_SERVER_LOG_FILE}"
  rm -f "${HOST_DBFS_OPERATOR_LOG_FILE}"
  rm -f "${HOST_DBFS_BUFFER_POOL_EVENT_LOG_FILE}"
fi
...
if [[ "${ENGINE}" == "dbfs" ]]; then
  ...
  echo "  ${HOST_DBFS_BUFFER_POOL_EVENT_LOG_FILE}"
else
